...
import { createCircle, createLongdoMarker, createObject, createPolygon, createPolyline, createPopup, createTMSLayer, createWMSLayer, EventName, LocationMode, longdoLayer, Map, OverlaysClickEvent } from "longdomap-react";
...
export default function MapCtrl({ map }: MapCtrlProps) {
    ...
    return (
        <aside className="w-full lg:w-72 bg-white shadow p-3 flex flex-col gap-2 rounded-lg text-black">
            ...
            <div className="bg-gray-100 p-2 text-sm">
                ...
                <span className="ml-2">
                    {overlayClick ? (
                        <span className="text-blue-700">{overlayClick._geojson.properties.title}</span>
                    ) : (
                        <span className="text-gray-500">Not set</span>
                    )}
                </span>
                ...
                <button onClick={() => {
                    if (map) {
                        ...
                        const popup = createPopup({
                            lat: 13.694 + randomOffset(),
                            lon: 100.747 + randomOffset(),
                        }, {
                            html: `<h1>Popup Example</h1><p>This is a popup example.</p>`,
                        })


                        const customPopup = createPopup({
                            lat: 13.695 + randomOffset(),
                            lon: 100.7527 + randomOffset(),
                        }, {
                            size: {
                                height: 100,
                                width: 200
                            },
                            title: `Custom Popup ${Math.floor(Math.random() * 1000)}`,
                            closable: true,
                            loadDetail: (e) => {
                                e.innerHTML = `<h1>Hello World</h1>`
                            }
                        })


                        map.Overlays.add(marker);
                        map.Overlays.add(customPopup);
            ...
            <button onClick={() => {
                map?.Overlays.clear();
                const krabi = createObject("81", "IG")
                const phuket = createObject("83", "IG")
                map?.Overlays.load(krabi)
                map?.Overlays.load(phuket)
                map?.location({
                    lat: 8.05159,
                    lon: 98.844194

                })
                setActiveButton("krabiphuket");
            }} className={`${activeButton === "krabiphuket" ? "bg-green-600" : "bg-blue-600"} text-white rounded px-2 py-1 text-sm text-left hover:cursor-pointer w-full`}>
                Pluket & Krabi
            </button>
